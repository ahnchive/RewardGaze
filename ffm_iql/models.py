...
from common.utils import get_foveal_weights
import math
...
class PositionalEncoding2D(nn.Module):
    ...
    def forward(self, fix_ind_map):
        # Convert fixation index map to positional embeddings
        bs, h, w = fix_ind_map.shape
        pe_map = torch.zeros(bs, self.d_model * 2, h, w, dtype=torch.float32).to(
            fix_ind_map.device)
        pe_i, pe_yi, pe_xi = torch.where(fix_ind_map > 0)
        pe_x = self.pos_w_embedding[pe_xi]
        pe_y = self.pos_h_embedding[pe_yi]
        pe_map[pe_i, :, pe_yi, pe_xi] = torch.cat([pe_x, pe_y], dim=1)
        return pe_map


# DCB model for IQ-Learn
class DCB_IQL(nn.Module):
    def __init__(self,
                 num_tasks,
                 task_emb_size,
                 task_eye,
                 ...
                 enhance_DCB="none",
                 forgetting_rate=0):
        super(DCB_IQL, self).__init__()
        if old_impl:
            task_emb_size = num_tasks
        self.enhance_DCB = enhance_DCB
        self.forgetting_rate = forgetting_rate
        assert self.enhance_DCB in [
            "none", "spatial", "temporal", "spatiotemporal"
        ], "not supported enhance_DCB type"

        self.max_pool = nn.MaxPool2d(2)
        self.feat_enc = nn.Sequential(
            nn.Conv2d(ch + task_emb_size, 128, 5, padding=2),
            nn.LayerNorm([128, 20, 32]),
            nn.ELU(),
            nn.Dropout(dropout),
        )
        ...
        self.actor1 = nn.Sequential(
            nn.Conv2d(128 + task_emb_size, 64, 3, padding=1),
            nn.LayerNorm([64, 20, 32]),
            nn.ELU(),
            nn.Dropout(dropout),
        )
        self.actor2 = nn.Sequential(
            nn.Conv2d(64 + task_emb_size, 32, 3, padding=1),
            nn.LayerNorm([32, 20, 32]),
            nn.ELU(),
            nn.Dropout(dropout),
        )
        self.actor3 = nn.Conv2d(32 + task_emb_size, 1, 1)

        self.task_eye = task_eye

        self.task_embed = nn.Embedding(num_tasks, task_emb_size)
        self.old_impl = old_impl

        self.fix_loc_embed = PositionalEncoding2D(ch + task_emb_size, dropout=dropout)
        self.fix_ind_embed = nn.Embedding(max_num_fixs + 2, ch + task_emb_size)
        # self.IOR_filters = nn.Parameter(torch.ones(1, 1, 3, 3))

    def get_one_hot(self, tid):
        task_onehot = self.task_eye.to(tid.device)[tid]
        return task_onehot

    def task_condition(self, x, tid):
        if self.old_impl:
            return modulate_features(x, tid)

        task_embs = self.task_embed.weight[tid]
        task_embs = task_embs.view(task_embs.size(0), task_embs.size(1), 1, 1)
        task_embs = task_embs.expand(task_embs.size(0), task_embs.size(1), x.size(2), x.size(3))
        x = torch.cat([x, task_embs], dim=1)
        return x

    def fixs2tokens(self, fix_map):
        """ convert fixation map to token map"""
        if self.enhance_DCB == "none":
            return 0
        elif self.enhance_DCB == "spatial":
            fix_map = fix_map.long().sum(dim=1)
            return self.fix_loc_embed(fix_map)
        elif self.enhance_DCB == "temporal":
            tempo_fix_map = (torch.arange(fix_map.shape[1]).to(
                fix_map.device).view(1, -1, 1, 1) + 1) * fix_map.long()
            fix_ind_embs = self.fix_ind_embed.weight[tempo_fix_map.view(
                -1)].view(*tempo_fix_map.shape, -1)
            fix_ind_embs[tempo_fix_map == 0] = 0  # 0th token is not used
            fix_ind_embs = fix_ind_embs.sum(dim=1)
            return fix_ind_embs.permute(0, 3, 1, 2)
        elif self.enhance_DCB == "spatiotemporal":
            fix_loc_embs = self.fix_loc_embed(fix_map.long().sum(dim=1))
            tempo_fix_map = (torch.arange(fix_map.shape[1]).to(
                fix_map.device).view(1, -1, 1, 1) + 1) * fix_map.long()
            fix_ind_embs = self.fix_ind_embed.weight[tempo_fix_map.view(
                -1)].view(*tempo_fix_map.shape, -1)
            fix_ind_embs[tempo_fix_map == 0] = 0  # 0th token is not used
            fix_ind_embs = fix_ind_embs.sum(dim=1)
            return fix_loc_embs + fix_ind_embs.permute(0, 3, 1, 2)
        else:
            raise NotImplementedError

    def forward(self, x, fix_map, tid, force_IOR=True):
        """ output the action probability"""
        bs, _, h, w = x.size()
        if self.old_impl:
            tid_onehot = self.get_one_hot(tid)
            tid = tid_onehot.view(bs, tid_onehot.size(1), 1, 1)

        x = self.task_condition(x, tid)
        x += self.fixs2tokens(fix_map)
        x = torch.relu(self.feat_enc(x))
        x = self.task_condition(x, tid)
        act_logits = torch.relu(self.actor1(x))
        act_logits = self.task_condition(act_logits, tid)
        act_logits = torch.relu(self.actor2(act_logits))
        act_logits = self.task_condition(act_logits, tid)
        q_values = self.actor3(act_logits).view(bs, -1)

        # Inhibition of return
        if not self.training and force_IOR and self.forgetting_rate > 0:
            IOR_mask = F.conv2d(fix_map.float(),
                                torch.ones(1, 1, 3, 3).to(fix_map.device), #self.IOR_filters, #
                                padding=1).view(bs, -1)
            # IOR_mask = (IOR_mask > 0).float()
            q_values = q_values - torch.abs(q_values * IOR_mask) * self.forgetting_rate
            # q_values[IOR_mask > 0] = -1
        return q_values