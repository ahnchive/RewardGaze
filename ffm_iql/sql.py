...
class SoftQ(object):
    def __init__(self, q_net, batch_size, device, args):
        ...
        self.args = args
        self.actor = None
        self.forgetting_rate = args.forgetting_rate
        ...
        self.critic_optimizer = AdamW(self.q_net.parameters(), lr=args.adam_lr,
    ...
    def get_Qs(self, state, action_mask=None):
        q = self.q_net(*state)
        if self.forgetting_rate > 0 and action_mask is not None:  # Soft inhibition of return
            q = q - torch.abs(q * self.forgetting_rate * action_mask.float())
        return q
    ...
    def select_action(self,
                      ...
                      sample_stop=False):
        with torch.no_grad():
            # q = self.get_Qs(state, action_mask)
            q = self.q_net(*state)
            probs = F.softmax(q/self.alpha, dim=1) # [32, 640]
        if sample_action:
            if action_mask is not None and self.forgetting_rate == 0:
