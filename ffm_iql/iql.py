...
def get_concat_samples(policy_batch, expert_batch, only_expert_states, device):
    ...
    if only_expert_states:
        ...
        return expert_batch_state, expert_batch_next_state, online_batch_action, expert_batch_done, is_expert
    ...
    batch_done = torch.cat([online_batch_done, expert_batch_done],
                           dim=0).to(device)
    if expert_batch_aux is None:
        batch_aux = None
    else:
        batch_aux = torch.cat([online_batch_aux, expert_batch_aux], dim=0).to(device)
    is_expert = torch.cat([
...
def irl_update_critic(self,
    ...
    if args.type == "sqil":
        ...
        with torch.no_grad():
            if args.div == "hellinger":
                ...
                phi_grad = torch.softmax(-reward, dim=0) * reward.shape[0]
    ...
    if args.regularize:
        ...
        current_Q = self.critic(obs, action)
        reward = current_Q - y
        # reg_loss = 1 / (4 * args.alpha) * (reward**2).mean()  # chi-2 loss
        # Bounded range loss (-1, 1)
        reg_loss = (1e5 * (reward > 1).float() + 1e5 * (reward < -1).float()).mean()
        # reg_loss = -reward[reward < 0].mean()  # [0, +inf]
        # reg_loss = (1e5 * (reward < 0).float()).mean()  # [0, +inf]
        loss += reg_loss
        losses['regularize_loss'] = reg_loss.item()
