...
def compute_SS(preds, clusters, truncate, truncate_gt, reduce='mean'):
    ...
    for scanpath in preds:
        ...
        if is_fv:
            pkey = '{}-{}'.format(scanpath['condition'], scanpath['name'].split('.')[0])
        else:
            pkey = '{}-{}-{}'.format(scanpath['condition'], scanpath['task'],
                                         scanpath['name'].split('.')[0])
        
        
        indices = [index for index, element in enumerate(clusters.keys()) if pkey in element]
        assert len(indices)==1
        key = list(clusters.keys())[indices[0]]
...
def compute_SSS(preds,
    ...
    for scanpath in preds:
        ...
        indices = [index for index, element in enumerate(fixations.keys()) if pkey in element]        
        assert len(indices)==1
        key = list(fixations.keys())[indices[0]]
