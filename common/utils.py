...
def get_num_steps(trajs, target_annos, task_names):
    ...
    for task in task_names:
        task_trajs = list(filter(lambda x: x['task'] == task, trajs))
        num_steps_task = np.ones(len(task_trajs), dtype=np.uint16)
        for i, traj in enumerate(task_trajs):
            key = traj['task'] + '_' + traj['name']
            bbox = target_annos[key]
...
def compute_search_cdf(scanpaths, annos, max_step, return_by_task=False, return_by_image=False):
    # compute search CDF
    task_names = np.unique([traj['task'] for traj in scanpaths])
    num_steps, num_steps_byimage = get_num_steps(scanpaths, annos, task_names)
    cdf_tasks = get_mean_cdf(num_steps, task_names, max_step) # previously max_step
    ...
    if return_by_image:
        cdf_byimage=[]
        for key in num_steps_byimage.keys():
            cdf_byimage.append(get_CDF(num_steps_byimage[key], max_step)) # be careful if maxstep or maxstep+1
        mean_cdf = np.mean(cdf_tasks, axis=0)
        dict_img2cdf = dict(zip(list(num_steps_byimage.keys()), cdf_byimage))
        return mean_cdf, dict_img2cdf
...
def actions2scanpaths(actions_all, patch_num, center_first_fix=True):
...
def preprocess_fixations(trajs,
    ...
    for traj in trajs:
        if not is_osie:  # For COCO-Search/Freeview datasets, force the initial fixation to be the center
        ...
        if is_osie and not sample_scanpath:
            # Predict the initial fixation for OSIE dataset
            if has_spid:
                fix_label = [traj['name'], traj['task'], traj['condition'],
                             [], label, False, traj['subject'],
                             traj['scanpath_id'], 0]
            else:
                fix_label = [traj['name'], traj['task'], traj['condition'],
                             [], label, False, traj['subject'], 0]
...
def transform_fixations(normalized_fixations, is_padding, pa, sample_scanpath=True, return_highres=False):
    ...
    fix_seq = transform(normalized_fixations, is_padding, [pa.im_w//32, pa.im_h//32], sample_scanpath)
    if return_highres:
        fix_seq_high = transform(normalized_fixations, is_padding, [pa.im_w//4, pa.im_h//4], sample_scanpath)
